    ...
    for (int i = 0; i < args.size(); i++) {
        if (args[i].compare("-v") == 0) {
            ...
            while (!file.eof()) {
                ...
                //insere qualificacoes no veiculos
                vector <qualificacoes> qualis;
                for (int java = 0; java < vetorQualis.size(); java++) {
                    if (!vetorQualis[java].getSiglaVeiculo().compare(sigla)) {
                        qualis.push_back(vetorQualis[java]);
                    }
                }
                
                
                
                veiculo.setQualis(qualis);                   
